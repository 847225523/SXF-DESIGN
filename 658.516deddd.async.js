(self.webpackChunkSXF_DESIGN = self.webpackChunkSXF_DESIGN || []).push([
  [658],
  {
    50373: function (M, N, a) {
      'use strict';
      a.r(N),
        a.d(N, {
          default: function () {
            return T;
          },
        });
      var e = a(50959),
        f = Object.defineProperty,
        g = Object.getOwnPropertySymbols,
        j = Object.prototype.hasOwnProperty,
        A = Object.prototype.propertyIsEnumerable,
        E = (s, l, c) =>
          l in s
            ? f(s, l, { enumerable: !0, configurable: !0, writable: !0, value: c })
            : (s[l] = c),
        S = (s, l) => {
          for (var c in l || (l = {})) j.call(l, c) && E(s, c, l[c]);
          if (g) for (var c of g(l)) A.call(l, c) && E(s, c, l[c]);
          return s;
        };
      const b = (s) =>
        e.createElement(
          'svg',
          S({ viewBox: '64 64 896 896' }, s),
          e.createElement('path', {
            d: 'M120 230h496c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8zm0 424h496c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8zm784 140H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8zm0-424H120c-4.4 0-8 3.6-8 8v56c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-56c0-4.4-3.6-8-8-8z',
          }),
        );
      var O =
          'data:image/svg+xml;base64,',
        C = function (s, l, c, m) {
          function k(y) {
            return y instanceof c
              ? y
              : new c(function (Q) {
                  Q(y);
                });
          }
          return new (c || (c = Promise))(function (y, Q) {
            function X(z) {
              try {
                oe(m.next(z));
              } catch ($) {
                Q($);
              }
            }
            function ae(z) {
              try {
                oe(m.throw(z));
              } catch ($) {
                Q($);
              }
            }
            function oe(z) {
              z.done ? y(z.value) : k(z.value).then(X, ae);
            }
            oe((m = m.apply(s, l || [])).next());
          });
        };
      function x(s) {
        let l = 0,
          c = 0,
          m = s;
        do (l += m.offsetTop || 0), (c += m.offsetLeft || 0), (m = m.offsetParent);
        while (m);
        return { top: l, left: c };
      }
      class J {
        constructor(l) {
          this.element = l;
        }
        getHorizontalScroll() {
          return this.element.scrollLeft;
        }
        getVerticalScroll() {
          return this.element.scrollTop;
        }
        getMaxHorizontalScroll() {
          return this.element.scrollWidth - this.element.clientWidth;
        }
        getMaxVerticalScroll() {
          return this.element.scrollHeight - this.element.clientHeight;
        }
        getHorizontalElementScrollOffset(l, c) {
          return x(l).left - x(c).left;
        }
        getVerticalElementScrollOffset(l, c) {
          return x(l).top - x(c).top;
        }
        scrollTo(l, c) {
          (this.element.scrollLeft = l), (this.element.scrollTop = c);
        }
      }
      class B {
        constructor() {
          this.element = window;
        }
        getHorizontalScroll() {
          return window.scrollX || document.documentElement.scrollLeft;
        }
        getVerticalScroll() {
          return window.scrollY || document.documentElement.scrollTop;
        }
        getMaxHorizontalScroll() {
          return (
            Math.max(
              document.body.scrollWidth,
              document.documentElement.scrollWidth,
              document.body.offsetWidth,
              document.documentElement.offsetWidth,
              document.body.clientWidth,
              document.documentElement.clientWidth,
            ) - window.innerWidth
          );
        }
        getMaxVerticalScroll() {
          return (
            Math.max(
              document.body.scrollHeight,
              document.documentElement.scrollHeight,
              document.body.offsetHeight,
              document.documentElement.offsetHeight,
              document.body.clientHeight,
              document.documentElement.clientHeight,
            ) - window.innerHeight
          );
        }
        getHorizontalElementScrollOffset(l) {
          return (
            (window.scrollX || document.documentElement.scrollLeft) + l.getBoundingClientRect().left
          );
        }
        getVerticalElementScrollOffset(l) {
          return (
            (window.scrollY || document.documentElement.scrollTop) + l.getBoundingClientRect().top
          );
        }
        scrollTo(l, c) {
          window.scrollTo(l, c);
        }
      }
      const I = {
          elements: [],
          cancelMethods: [],
          add: (s, l) => {
            I.elements.push(s), I.cancelMethods.push(l);
          },
          remove: (s, l) => {
            const c = I.elements.indexOf(s);
            c > -1 &&
              (l && I.cancelMethods[c](), I.elements.splice(c, 1), I.cancelMethods.splice(c, 1));
          },
        },
        v = typeof window != 'undefined',
        h = {
          cancelOnUserAction: !0,
          easing: (s) => --s * s * s + 1,
          elementToScroll: v ? window : null,
          horizontalOffset: 0,
          maxDuration: 3e3,
          minDuration: 250,
          speed: 500,
          verticalOffset: 0,
        };
      function D(s, l = {}) {
        return C(this, void 0, void 0, function* () {
          if (v) {
            if (!window.Promise)
              throw "Browser doesn't support Promises, and animated-scroll-to depends on it, please provide a polyfill.";
          } else
            return new Promise((ye) => {
              ye(!1);
            });
          let c,
            m,
            k,
            y = Object.assign(Object.assign({}, h), l);
          const Q = y.elementToScroll === window,
            X = !!y.elementToScroll.nodeName;
          if (!Q && !X) throw 'Element to scroll needs to be either window or DOM element.';
          const ae = Q ? document.documentElement : y.elementToScroll;
          getComputedStyle(ae).getPropertyValue('scroll-behavior') === 'smooth' &&
            console.warn(
              `${ae.tagName} has "scroll-behavior: smooth" which can mess up with animated-scroll-to's animations`,
            );
          const z = Q ? new B() : new J(y.elementToScroll);
          if (s instanceof Element) {
            if (((k = s), X && (!y.elementToScroll.contains(k) || y.elementToScroll.isSameNode(k))))
              throw 'options.elementToScroll has to be a parent of scrollToElement';
            (c = z.getHorizontalElementScrollOffset(k, y.elementToScroll)),
              (m = z.getVerticalElementScrollOffset(k, y.elementToScroll));
          } else if (typeof s == 'number') (c = z.getHorizontalScroll()), (m = s);
          else if (Array.isArray(s) && s.length === 2)
            (c = s[0] === null ? z.getHorizontalScroll() : s[0]),
              (m = s[1] === null ? z.getVerticalScroll() : s[1]);
          else
            throw `Wrong function signature. Check documentation.
Available method signatures are:
  animateScrollTo(y:number, options)
  animateScrollTo([x:number | null, y:number | null], options)
  animateScrollTo(scrollToElement:Element, options)`;
          (c += y.horizontalOffset), (m += y.verticalOffset);
          const $ = z.getMaxHorizontalScroll(),
            ue = z.getHorizontalScroll();
          c > $ && (c = $);
          const Y = c - ue,
            R = z.getMaxVerticalScroll(),
            _ = z.getVerticalScroll();
          m > R && (m = R);
          const q = m - _,
            le = Math.abs(Math.round((Y / 1e3) * y.speed)),
            me = Math.abs(Math.round((q / 1e3) * y.speed));
          let Me = le > me ? le : me;
          return (
            Me < y.minDuration ? (Me = y.minDuration) : Me > y.maxDuration && (Me = y.maxDuration),
            new Promise((ye, Ce) => {
              Y === 0 && q === 0 && ye(!0), I.remove(z.element, !0);
              let Ie;
              const Qe = () => {
                o(), cancelAnimationFrame(Ie), ye(!1);
              };
              I.add(z.element, Qe);
              const be = (w) => w.preventDefault(),
                xe = y.cancelOnUserAction ? Qe : be,
                Ye = y.cancelOnUserAction ? { passive: !0 } : { passive: !1 },
                ke = ['wheel', 'touchstart', 'keydown', 'mousedown'],
                o = () => {
                  ke.forEach((w) => {
                    z.element.removeEventListener(w, xe, Ye);
                  });
                };
              ke.forEach((w) => {
                z.element.addEventListener(w, xe, Ye);
              });
              const u = Date.now(),
                i = () => {
                  var w = Date.now() - u,
                    V = w / Me;
                  const Z = Math.round(ue + Y * y.easing(V)),
                    W = Math.round(_ + q * y.easing(V));
                  w < Me && (Z !== c || W !== m)
                    ? (z.scrollTo(Z, W), (Ie = requestAnimationFrame(i)))
                    : (z.scrollTo(c, m),
                      cancelAnimationFrame(Ie),
                      o(),
                      I.remove(z.element, !1),
                      ye(!0));
                };
              Ie = requestAnimationFrame(i);
            })
          );
        });
      }
      var U = D,
        H = a(3989),
        K = a(50335),
        d = a(30927),
        p = a(96761),
        L = a(26389),
        P = a(38609),
        F = a(92627),
        G = a(68282);
      function te(s, l) {
        return De(s) || Le(s, l) || se(s, l) || re();
      }
      function re() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function se(s, l) {
        if (s) {
          if (typeof s == 'string') return ge(s, l);
          var c = Object.prototype.toString.call(s).slice(8, -1);
          if (
            (c === 'Object' && s.constructor && (c = s.constructor.name),
            c === 'Map' || c === 'Set')
          )
            return Array.from(s);
          if (c === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(c))
            return ge(s, l);
        }
      }
      function ge(s, l) {
        (l == null || l > s.length) && (l = s.length);
        for (var c = 0, m = new Array(l); c < l; c++) m[c] = s[c];
        return m;
      }
      function Le(s, l) {
        var c =
          s == null
            ? null
            : (typeof Symbol != 'undefined' && s[Symbol.iterator]) || s['@@iterator'];
        if (c != null) {
          var m = [],
            k = !0,
            y = !1,
            Q,
            X;
          try {
            for (
              c = c.call(s);
              !(k = (Q = c.next()).done) && (m.push(Q.value), !(l && m.length === l));
              k = !0
            );
          } catch (ae) {
            (y = !0), (X = ae);
          } finally {
            try {
              !k && c.return != null && c.return();
            } finally {
              if (y) throw X;
            }
          }
          return m;
        }
      }
      function De(s) {
        if (Array.isArray(s)) return s;
      }
      var Oe = function () {
          var l = (0, H.YB)(),
            c = (0, H.pC)(),
            m = (0, H.tx)(),
            k = (0, H.TH)(),
            y = k.hash,
            Q = (0, H.WF)(),
            X = Q.loading,
            ae = (0, e.useState)(!1),
            oe = te(ae, 2),
            z = oe[0],
            $ = oe[1],
            ue = (0, H.eL)(),
            Y = ue.frontmatter,
            R = Y.sidebar !== !1 && (m == null ? void 0 : m.length) > 0;
          return (
            (0, e.useEffect)(
              function () {
                var _ = y.replace('#', '');
                _ &&
                  setTimeout(function () {
                    var q = document.getElementById(decodeURIComponent(_));
                    q && U(q.offsetTop - 80, { maxDuration: 300 });
                  }, 1);
              },
              [X, y],
            ),
            e.createElement(
              'div',
              {
                className: 'dumi-default-doc-layout',
                'data-mobile-sidebar-active': z || void 0,
                onClick: function () {
                  return $(!1);
                },
              },
              e.createElement(
                H.ql,
                null,
                e.createElement('html', { lang: l.locale.replace(/-.+$/, '') }),
                Y.title && e.createElement('title', null, Y.title),
                Y.title && e.createElement('meta', { property: 'og:title', content: Y.title }),
                Y.description &&
                  e.createElement('meta', { name: 'description', content: Y.description }),
                Y.description &&
                  e.createElement('meta', { property: 'og:description', content: Y.description }),
                Y.keywords &&
                  e.createElement('meta', { name: 'keywords', content: Y.keywords.join(',') }),
                Y.keywords &&
                  e.createElement('meta', {
                    property: 'og:keywords',
                    content: Y.keywords.join(','),
                  }),
              ),
              e.createElement(L.Z, null),
              e.createElement(P.Z, null),
              e.createElement(d.Z, null),
              R &&
                e.createElement(
                  'div',
                  { className: 'dumi-default-doc-layout-mobile-bar' },
                  e.createElement(
                    'button',
                    {
                      type: 'button',
                      className: 'dumi-default-sidebar-btn',
                      onClick: function (q) {
                        q.stopPropagation(),
                          $(function (le) {
                            return !le;
                          });
                      },
                    },
                    e.createElement(b, null),
                    l.formatMessage({ id: 'layout.sidebar.btn' }),
                  ),
                ),
              e.createElement(
                'main',
                null,
                R && e.createElement(F.Z, null),
                e.createElement(K.Z, null, c, e.createElement(p.Z, null)),
                Y.toc === 'content' &&
                  e.createElement(
                    'div',
                    { className: 'dumi-default-doc-layout-toc-wrapper' },
                    e.createElement('h4', null, 'TABLE OF CONTENTS'),
                    e.createElement(G.Z, null),
                  ),
              ),
            )
          );
        },
        T = Oe;
    },
    95034: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return I;
        },
      });
      var e = a(3989),
        f = a(50959);
      function g(v, h) {
        return b(v) || S(v, h) || A(v, h) || j();
      }
      function j() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function A(v, h) {
        if (v) {
          if (typeof v == 'string') return E(v, h);
          var D = Object.prototype.toString.call(v).slice(8, -1);
          if (
            (D === 'Object' && v.constructor && (D = v.constructor.name),
            D === 'Map' || D === 'Set')
          )
            return Array.from(v);
          if (D === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(D))
            return E(v, h);
        }
      }
      function E(v, h) {
        (h == null || h > v.length) && (h = v.length);
        for (var D = 0, U = new Array(h); D < h; D++) U[D] = v[D];
        return U;
      }
      function S(v, h) {
        var D =
          v == null
            ? null
            : (typeof Symbol != 'undefined' && v[Symbol.iterator]) || v['@@iterator'];
        if (D != null) {
          var U = [],
            H = !0,
            K = !1,
            d,
            p;
          try {
            for (
              D = D.call(v);
              !(H = (d = D.next()).done) && (U.push(d.value), !(h && U.length === h));
              H = !0
            );
          } catch (L) {
            (K = !0), (p = L);
          } finally {
            try {
              !H && D.return != null && D.return();
            } finally {
              if (K) throw p;
            }
          }
          return U;
        }
      }
      function b(v) {
        if (Array.isArray(v)) return v;
      }
      var O = function () {
          return f.createElement(
            'svg',
            { viewBox: '0 0 16 16' },
            f.createElement('path', {
              d: 'M8.218 1.455c3.527.109 6.327 3.018 6.327 6.545 0 3.6-2.945 6.545-6.545 6.545a6.562 6.562 0 0 1-6.036-4h.218c3.6 0 6.545-2.945 6.545-6.545 0-.91-.182-1.745-.509-2.545m0-1.455c-.473 0-.909.218-1.2.618-.29.4-.327.946-.145 1.382.254.655.4 1.31.4 2 0 2.8-2.291 5.09-5.091 5.09h-.218c-.473 0-.91.22-1.2.62-.291.4-.328.945-.146 1.38C1.891 14.074 4.764 16 8 16c4.4 0 8-3.6 8-8a7.972 7.972 0 0 0-7.745-8h-.037Z',
            }),
          );
        },
        C = function () {
          return f.createElement(
            'svg',
            { viewBox: '0 0 16 16' },
            f.createElement('path', {
              d: 'M8 13a1 1 0 0 1 1 1v1a1 1 0 1 1-2 0v-1a1 1 0 0 1 1-1ZM8 3a1 1 0 0 1-1-1V1a1 1 0 1 1 2 0v1a1 1 0 0 1-1 1Zm7 4a1 1 0 1 1 0 2h-1a1 1 0 1 1 0-2h1ZM3 8a1 1 0 0 1-1 1H1a1 1 0 1 1 0-2h1a1 1 0 0 1 1 1Zm9.95 3.536.707.707a1 1 0 0 1-1.414 1.414l-.707-.707a1 1 0 0 1 1.414-1.414Zm-9.9-7.072-.707-.707a1 1 0 0 1 1.414-1.414l.707.707A1 1 0 0 1 3.05 4.464Zm9.9 0a1 1 0 0 1-1.414-1.414l.707-.707a1 1 0 0 1 1.414 1.414l-.707.707Zm-9.9 7.072a1 1 0 0 1 1.414 1.414l-.707.707a1 1 0 0 1-1.414-1.414l.707-.707ZM8 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm0 6.5a2.5 2.5 0 1 1 0-5 2.5 2.5 0 0 1 0 5Z',
            }),
          );
        },
        x = function () {
          return f.createElement(
            'svg',
            { viewBox: '0 0 16 16' },
            f.createElement('path', {
              d: 'M14.595 8a6.595 6.595 0 1 1-13.19 0 6.595 6.595 0 0 1 13.19 0ZM8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0Zm0 2.014v11.972A5.986 5.986 0 0 0 8 2.014Z',
            }),
          );
        },
        J = { light: C, dark: O, auto: x },
        B = function () {
          var h = (0, e.WF)(),
            D = h.themeConfig.prefersColor.default,
            U = (0, e.YB)(),
            H = (0, e.OI)(),
            K = g(H, 3),
            d = K[1],
            p = d === void 0 ? D : d,
            L = K[2],
            P = J[p];
          return f.createElement(
            'span',
            {
              className: 'dumi-default-color-switch',
              'data-dumi-tooltip': U.formatMessage({ id: 'header.color.mode.'.concat(p) }),
              'data-dumi-tooltip-bottom': !0,
            },
            P && f.createElement(P, null),
            f.createElement(
              'select',
              {
                onChange: function (G) {
                  return L(G.target.value);
                },
                value: p,
              },
              ['light', 'dark', 'auto'].map(function (F) {
                return f.createElement(
                  'option',
                  { value: F, key: F },
                  U.formatMessage({ id: 'header.color.mode.'.concat(F) }),
                );
              }),
            ),
          );
        },
        I = B;
    },
    53299: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return j;
        },
      });
      var e = a(3989),
        f = a(50959),
        g = function (E) {
          var S = (0, e.tx)();
          return f.createElement(
            'div',
            { className: 'dumi-default-content', 'data-no-sidebar': !S || void 0 },
            E.children,
          );
        },
        j = g;
    },
    89639: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return j;
        },
      });
      var e = a(3989),
        f = a(50959),
        g = function () {
          var E,
            S = (0, e.eL)(),
            b = S.frontmatter;
          return Boolean((E = b.features) === null || E === void 0 ? void 0 : E.length)
            ? f.createElement(
                'div',
                {
                  className: 'dumi-default-features',
                  'data-cols':
                    [3, 2].find(function (O) {
                      return b.features.length % O === 0;
                    }) || 3,
                },
                b.features.map(function (O) {
                  var C = O.title,
                    x = O.description,
                    J = O.emoji,
                    B = O.link,
                    I;
                  return (
                    B &&
                      (I = /^(\w+:)\/\/|^(mailto|tel):/.test(B)
                        ? f.createElement('a', { href: B, target: '_blank', rel: 'noreferrer' }, C)
                        : f.createElement(e.rU, { to: B }, C)),
                    f.createElement(
                      'div',
                      { key: C, className: 'dumi-default-features-item' },
                      J && f.createElement('i', null, J),
                      C && f.createElement('h3', null, I || C),
                      x && f.createElement('p', { dangerouslySetInnerHTML: { __html: x } }),
                    )
                  );
                }),
              )
            : null;
        },
        j = g;
    },
    84749: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return j;
        },
      });
      var e = a(3989),
        f = a(50959),
        g = function () {
          var E = (0, e.WF)(),
            S = E.themeConfig;
          return S.footer
            ? f.createElement('div', {
                className: 'dumi-default-footer',
                dangerouslySetInnerHTML: { __html: S.footer },
              })
            : null;
        },
        j = g;
    },
    65204: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return ut;
        },
      });
      var e = a(50959),
        f = function () {
          return e.createElement(e.Fragment, null);
        },
        g = f,
        j = Object.defineProperty,
        A = Object.getOwnPropertySymbols,
        E = Object.prototype.hasOwnProperty,
        S = Object.prototype.propertyIsEnumerable,
        b = (n, t, r) =>
          t in n
            ? j(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        O = (n, t) => {
          for (var r in t || (t = {})) E.call(t, r) && b(n, r, t[r]);
          if (A) for (var r of A(t)) S.call(t, r) && b(n, r, t[r]);
          return n;
        };
      const C = (n) =>
        e.createElement(
          'svg',
          O({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'm563.8 512 262.5-312.9c4.4-5.2.7-13.1-6.1-13.1h-79.8c-4.7 0-9.2 2.1-12.3 5.7L511.6 449.8 295.1 191.7c-3-3.6-7.5-5.7-12.3-5.7H203c-6.8 0-10.5 7.9-6.1 13.1L459.4 512 196.9 824.9A7.95 7.95 0 0 0 203 838h79.8c4.7 0 9.2-2.1 12.3-5.7l216.5-258.1 216.5 258.1c3 3.6 7.5 5.7 12.3 5.7h79.8c6.8 0 10.5-7.9 6.1-13.1L563.8 512z',
          }),
        );
      var x =
          'data:image/svg+xml;base64,',
        J = Object.defineProperty,
        B = Object.getOwnPropertySymbols,
        I = Object.prototype.hasOwnProperty,
        v = Object.prototype.propertyIsEnumerable,
        h = (n, t, r) =>
          t in n
            ? J(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        D = (n, t) => {
          for (var r in t || (t = {})) I.call(t, r) && h(n, r, t[r]);
          if (B) for (var r of B(t)) v.call(t, r) && h(n, r, t[r]);
          return n;
        };
      const U = (n) =>
        e.createElement(
          'svg',
          D({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M904 160H120c-4.4 0-8 3.6-8 8v64c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-64c0-4.4-3.6-8-8-8zm0 624H120c-4.4 0-8 3.6-8 8v64c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-64c0-4.4-3.6-8-8-8zm0-312H120c-4.4 0-8 3.6-8 8v64c0 4.4 3.6 8 8 8h784c4.4 0 8-3.6 8-8v-64c0-4.4-3.6-8-8-8z',
          }),
        );
      var H =
          'data:image/svg+xml;base64,',
        K = a(3989),
        d = a(41125),
        p = a(10303),
        L = a(93607),
        P = a(3569),
        F = a(67757),
        G = a(68222),
        te = Object.defineProperty,
        re = Object.getOwnPropertySymbols,
        se = Object.prototype.hasOwnProperty,
        ge = Object.prototype.propertyIsEnumerable,
        Le = (n, t, r) =>
          t in n
            ? te(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        De = (n, t) => {
          for (var r in t || (t = {})) se.call(t, r) && Le(n, r, t[r]);
          if (re) for (var r of re(t)) ge.call(t, r) && Le(n, r, t[r]);
          return n;
        };
      const Oe = (n) =>
        e.createElement(
          'svg',
          De({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M880 112H144c-17.7 0-32 14.3-32 32v736c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V144c0-17.7-14.3-32-32-32zm-32 736H663.9V602.2h104l15.6-120.7H663.9v-77.1c0-35 9.7-58.8 59.8-58.8h63.9v-108c-11.1-1.5-49-4.8-93.2-4.8-92.2 0-155.3 56.3-155.3 159.6v89H434.9v120.7h104.3V848H176V176h672v672z',
          }),
        );
      var T =
          'data:image/svg+xml;base64,',
        s = Object.defineProperty,
        l = Object.getOwnPropertySymbols,
        c = Object.prototype.hasOwnProperty,
        m = Object.prototype.propertyIsEnumerable,
        k = (n, t, r) =>
          t in n
            ? s(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        y = (n, t) => {
          for (var r in t || (t = {})) c.call(t, r) && k(n, r, t[r]);
          if (l) for (var r of l(t)) m.call(t, r) && k(n, r, t[r]);
          return n;
        };
      const Q = (n) =>
        e.createElement(
          'svg',
          y({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M511.6 76.3C264.3 76.2 64 276.4 64 523.5 64 718.9 189.3 885 363.8 946c23.5 5.9 19.9-10.8 19.9-22.2v-77.5c-135.7 15.9-141.2-73.9-150.3-88.9C215 726 171.5 718 184.5 703c30.9-15.9 62.4 4 98.9 57.9 26.4 39.1 77.9 32.5 104 26 5.7-23.5 17.9-44.5 34.7-60.8-140.6-25.2-199.2-111-199.2-213 0-49.5 16.3-95 48.3-131.7-20.4-60.5 1.9-112.3 4.9-120 58.1-5.2 118.5 41.6 123.2 45.3 33-8.9 70.7-13.6 112.9-13.6 42.4 0 80.2 4.9 113.5 13.9 11.3-8.6 67.3-48.8 121.3-43.9 2.9 7.7 24.7 58.3 5.5 118 32.4 36.8 48.9 82.7 48.9 132.3 0 102.2-59 188.1-200 212.9a127.5 127.5 0 0 1 38.1 91v112.5c.8 9 0 17.9 15 17.9 177.1-59.7 304.6-227 304.6-424.1 0-247.2-200.4-447.3-447.5-447.3z',
          }),
        );
      var X =
          'data:image/svg+xml;base64,',
        ae = Object.defineProperty,
        oe = Object.getOwnPropertySymbols,
        z = Object.prototype.hasOwnProperty,
        $ = Object.prototype.propertyIsEnumerable,
        ue = (n, t, r) =>
          t in n
            ? ae(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        Y = (n, t) => {
          for (var r in t || (t = {})) z.call(t, r) && ue(n, r, t[r]);
          if (oe) for (var r of oe(t)) $.call(t, r) && ue(n, r, t[r]);
          return n;
        };
      const R = (n) =>
        e.createElement(
          'svg',
          Y({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M913.9 552.2 805 181.4v-.1c-7.6-22.9-25.7-36.5-48.3-36.5-23.4 0-42.5 13.5-49.7 35.2l-71.4 213H388.8l-71.4-213c-7.2-21.7-26.3-35.2-49.7-35.2-23.1 0-42.5 14.8-48.4 36.6L110.5 552.2c-4.4 14.7 1.2 31.4 13.5 40.7l368.5 276.4c2.6 3.6 6.2 6.3 10.4 7.8l8.6 6.4 8.5-6.4c4.9-1.7 9-4.7 11.9-8.9l368.4-275.4c12.4-9.2 18-25.9 13.6-40.6zM751.7 193.4c1-1.8 2.9-1.9 3.5-1.9 1.1 0 2.5.3 3.4 3L818 394.3H684.5l67.2-200.9zm-487.4 1c.9-2.6 2.3-2.9 3.4-2.9 2.7 0 2.9.1 3.4 1.7l67.3 201.2H206.5l57.8-200zM158.8 558.7l28.2-97.3 202.4 270.2-230.6-172.9zm73.9-116.4h122.1l90.8 284.3-212.9-284.3zM512.9 776 405.7 442.3H620L512.9 776zm157.9-333.7h119.5L580 723.1l90.8-280.8zm-40.7 293.9 207.3-276.7 29.5 99.2-236.8 177.5z',
          }),
        );
      var _ =
          'data:image/svg+xml;base64,',
        q = Object.defineProperty,
        le = Object.getOwnPropertySymbols,
        me = Object.prototype.hasOwnProperty,
        Me = Object.prototype.propertyIsEnumerable,
        ye = (n, t, r) =>
          t in n
            ? q(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        Ce = (n, t) => {
          for (var r in t || (t = {})) me.call(t, r) && ye(n, r, t[r]);
          if (le) for (var r of le(t)) Me.call(t, r) && ye(n, r, t[r]);
          return n;
        };
      const Ie = (n) =>
        e.createElement(
          'svg',
          Ce({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M847.7 112H176.3c-35.5 0-64.3 28.8-64.3 64.3v671.4c0 35.5 28.8 64.3 64.3 64.3h671.4c35.5 0 64.3-28.8 64.3-64.3V176.3c0-35.5-28.8-64.3-64.3-64.3zm0 736c-447.8-.1-671.7-.2-671.7-.3.1-447.8.2-671.7.3-671.7 447.8.1 671.7.2 671.7.3-.1 447.8-.2 671.7-.3 671.7zM230.6 411.9h118.7v381.8H230.6zm59.4-52.2c37.9 0 68.8-30.8 68.8-68.8a68.8 68.8 0 1 0-137.6 0c-.1 38 30.7 68.8 68.8 68.8zm252.3 245.1c0-49.8 9.5-98 71.2-98 60.8 0 61.7 56.9 61.7 101.2v185.7h118.6V584.3c0-102.8-22.2-181.9-142.3-181.9-57.7 0-96.4 31.7-112.3 61.7h-1.6v-52.2H423.7v381.8h118.6V604.8z',
          }),
        );
      var Qe =
          'data:image/svg+xml;base64,',
        be = Object.defineProperty,
        xe = Object.getOwnPropertySymbols,
        Ye = Object.prototype.hasOwnProperty,
        ke = Object.prototype.propertyIsEnumerable,
        o = (n, t, r) =>
          t in n
            ? be(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        u = (n, t) => {
          for (var r in t || (t = {})) Ye.call(t, r) && o(n, r, t[r]);
          if (xe) for (var r of xe(t)) ke.call(t, r) && o(n, r, t[r]);
          return n;
        };
      const i = (n) =>
        e.createElement(
          'svg',
          u({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M928 254.3c-30.6 13.2-63.9 22.7-98.2 26.4a170.1 170.1 0 0 0 75-94 336.64 336.64 0 0 1-108.2 41.2A170.1 170.1 0 0 0 672 174c-94.5 0-170.5 76.6-170.5 170.6 0 13.2 1.6 26.4 4.2 39.1-141.5-7.4-267.7-75-351.6-178.5a169.32 169.32 0 0 0-23.2 86.1c0 59.2 30.1 111.4 76 142.1a172 172 0 0 1-77.1-21.7v2.1c0 82.9 58.6 151.6 136.7 167.4a180.6 180.6 0 0 1-44.9 5.8c-11.1 0-21.6-1.1-32.2-2.6C211 652 273.9 701.1 348.8 702.7c-58.6 45.9-132 72.9-211.7 72.9-14.3 0-27.5-.5-41.2-2.1C171.5 822 261.2 850 357.8 850 671.4 850 843 590.2 843 364.7c0-7.4 0-14.8-.5-22.2 33.2-24.3 62.3-54.4 85.5-88.2z',
          }),
        );
      var w =
          'data:image/svg+xml;base64,',
        V = Object.defineProperty,
        Z = Object.getOwnPropertySymbols,
        W = Object.prototype.hasOwnProperty,
        ie = Object.prototype.propertyIsEnumerable,
        ne = (n, t, r) =>
          t in n
            ? V(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        de = (n, t) => {
          for (var r in t || (t = {})) W.call(t, r) && ne(n, r, t[r]);
          if (Z) for (var r of Z(t)) ie.call(t, r) && ne(n, r, t[r]);
          return n;
        };
      const ee = (n) =>
        e.createElement(
          'svg',
          de({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M457.3 543c-68.1-17.7-145 16.2-174.6 76.2-30.1 61.2-1 129.1 67.8 151.3 71.2 23 155.2-12.2 184.4-78.3 28.7-64.6-7.2-131-77.6-149.2zm-52 156.2c-13.8 22.1-43.5 31.7-65.8 21.6-22-10-28.5-35.7-14.6-57.2 13.7-21.4 42.3-31 64.4-21.7 22.4 9.5 29.6 35 16 57.3zm45.5-58.5c-5 8.6-16.1 12.7-24.7 9.1-8.5-3.5-11.2-13.1-6.4-21.5 5-8.4 15.6-12.4 24.1-9.1 8.7 3.2 11.8 12.9 7 21.5zm334.5-197.2c15 4.8 31-3.4 35.9-18.3 11.8-36.6 4.4-78.4-23.2-109a111.39 111.39 0 0 0-106-34.3 28.45 28.45 0 0 0-21.9 33.8 28.39 28.39 0 0 0 33.8 21.8c18.4-3.9 38.3 1.8 51.9 16.7a54.2 54.2 0 0 1 11.3 53.3 28.45 28.45 0 0 0 18.2 36zm99.8-206c-56.7-62.9-140.4-86.9-217.7-70.5a32.98 32.98 0 0 0-25.4 39.3 33.12 33.12 0 0 0 39.3 25.5c55-11.7 114.4 5.4 154.8 50.1 40.3 44.7 51.2 105.7 34 159.1-5.6 17.4 3.9 36 21.3 41.7 17.4 5.6 36-3.9 41.6-21.2v-.1c24.1-75.4 8.9-161.1-47.9-223.9zM729 499c-12.2-3.6-20.5-6.1-14.1-22.1 13.8-34.7 15.2-64.7.3-86-28-40.1-104.8-37.9-192.8-1.1 0 0-27.6 12.1-20.6-9.8 13.5-43.5 11.5-79.9-9.6-101-47.7-47.8-174.6 1.8-283.5 110.6C127.3 471.1 80 557.5 80 632.2 80 775.1 263.2 862 442.5 862c235 0 391.3-136.5 391.3-245 0-65.5-55.2-102.6-104.8-118zM443 810.8c-143 14.1-266.5-50.5-275.8-144.5-9.3-93.9 99.2-181.5 242.2-195.6 143-14.2 266.5 50.5 275.8 144.4C694.4 709 586 796.6 443 810.8z',
          }),
        );
      var Ae =
          'data:image/svg+xml;base64,',
        je = Object.defineProperty,
        ce = Object.getOwnPropertySymbols,
        pe = Object.prototype.hasOwnProperty,
        he = Object.prototype.propertyIsEnumerable,
        Ve = (n, t, r) =>
          t in n
            ? je(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        Ze = (n, t) => {
          for (var r in t || (t = {})) pe.call(t, r) && Ve(n, r, t[r]);
          if (ce) for (var r of ce(t)) he.call(t, r) && Ve(n, r, t[r]);
          return n;
        };
      const We = (n) =>
        e.createElement(
          'svg',
          Ze({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M854.6 370.6c-9.9-39.4 9.9-102.2 73.4-124.4l-67.9-3.6s-25.7-90-143.6-98c-117.8-8.1-194.9-3-195-3 .1 0 87.4 55.6 52.4 154.7-25.6 52.5-65.8 95.6-108.8 144.7-1.3 1.3-2.5 2.6-3.5 3.7C319.4 605 96 860 96 860c245.9 64.4 410.7-6.3 508.2-91.1 20.5-.2 35.9-.3 46.3-.3 135.8 0 250.6-117.6 245.9-248.4-3.2-89.9-31.9-110.2-41.8-149.6zm-204.1 334c-10.6 0-26.2.1-46.8.3l-23.6.2-17.8 15.5c-47.1 41-104.4 71.5-171.4 87.6-52.5 12.6-110 16.2-172.7 9.6 18-20.5 36.5-41.6 55.4-63.1 92-104.6 173.8-197.5 236.9-268.5l1.4-1.4 1.3-1.5c4.1-4.6 20.6-23.3 24.7-28.1 9.7-11.1 17.3-19.9 24.5-28.6 30.7-36.7 52.2-67.8 69-102.2l1.6-3.3 1.2-3.4c13.7-38.8 15.4-76.9 6.2-112.8 22.5.7 46.5 1.9 71.7 3.6 33.3 2.3 55.5 12.9 71.1 29.2 5.8 6 10.2 12.5 13.4 18.7 1 2 1.7 3.6 2.3 5l5 17.7c-15.7 34.5-19.9 73.3-11.4 107.2 3 11.8 6.9 22.4 12.3 34.4 2.1 4.7 9.5 20.1 11 23.3 10.3 22.7 15.4 43 16.7 78.7 3.3 94.6-82.7 181.9-182 181.9z',
          }),
        );
      var Ue =
          'data:image/svg+xml;base64,',
        fe = Object.defineProperty,
        we = Object.getOwnPropertySymbols,
        Ne = Object.prototype.hasOwnProperty,
        Pe = Object.prototype.propertyIsEnumerable,
        Fe = (n, t, r) =>
          t in n
            ? fe(n, t, { enumerable: !0, configurable: !0, writable: !0, value: r })
            : (n[t] = r),
        _e = (n, t) => {
          for (var r in t || (t = {})) Ne.call(t, r) && Fe(n, r, t[r]);
          if (we) for (var r of we(t)) Pe.call(t, r) && Fe(n, r, t[r]);
          return n;
        };
      const Ge = (n) =>
        e.createElement(
          'svg',
          _e({ viewBox: '64 64 896 896' }, n),
          e.createElement('path', {
            d: 'M564.7 230.1V803h60l25.2 71.4L756.3 803h131.5V230.1H564.7zm247.7 497h-59.9l-75.1 50.4-17.8-50.4h-18V308.3h170.7v418.8zM526.1 486.9H393.3c2.1-44.9 4.3-104.3 6.6-172.9h130.9l-.1-8.1c0-.6-.2-14.7-2.3-29.1-2.1-15-6.6-34.9-21-34.9H287.8c4.4-20.6 15.7-69.7 29.4-93.8l6.4-11.2-12.9-.7c-.8 0-19.6-.9-41.4 10.6-35.7 19-51.7 56.4-58.7 84.4-18.4 73.1-44.6 123.9-55.7 145.6-3.3 6.4-5.3 10.2-6.2 12.8-1.8 4.9-.8 9.8 2.8 13 10.5 9.5 38.2-2.9 38.5-3 .6-.3 1.3-.6 2.2-1 13.9-6.3 55.1-25 69.8-84.5h56.7c.7 32.2 3.1 138.4 2.9 172.9h-141l-2.1 1.5c-23.1 16.9-30.5 63.2-30.8 65.2l-1.4 9.2h167c-12.3 78.3-26.5 113.4-34 127.4-3.7 7-7.3 14-10.7 20.8-21.3 42.2-43.4 85.8-126.3 153.6-3.6 2.8-7 8-4.8 13.7 2.4 6.3 9.3 9.1 24.6 9.1 5.4 0 11.8-.3 19.4-1 49.9-4.4 100.8-18 135.1-87.6 17-35.1 31.7-71.7 43.9-108.9L497 850l5-12c.8-1.9 19-46.3 5.1-95.9l-.5-1.8-108.1-123-22 16.6c6.4-26.1 10.6-49.9 12.5-71.1h158.7v-8c0-40.1-18.5-63.9-19.2-64.9l-2.4-3z',
          }),
        );
      var Ke =
          'data:image/svg+xml;base64,',
        Je = a(66845),
        He = {
          github: Q,
          weibo: ee,
          twitter: i,
          gitlab: R,
          facebook: Oe,
          zhihu: Ge,
          yuque: We,
          linkedin: Ie,
        },
        $e = function (t) {
          var r = t.icon,
            ve = t.link,
            Se = (0, Je.Z)(),
            Ee = (0, e.useMemo)(
              function () {
                return { Icon: He[r], link: ve };
              },
              [r, ve],
            );
          return e.createElement(
            'a',
            {
              className: 'dumi-default-icon',
              'data-dumi-tooltip': Se.formatMessage({ id: 'header.social.'.concat(r) }),
              'data-dumi-tooltip-bottom': !0,
              target: '_blank',
              href: Ee.link,
              rel: 'noreferrer',
            },
            e.createElement(Ee.Icon, null),
          );
        },
        qe = $e;
      function et(n, t) {
        return at(n) || nt(n, t) || rt(n, t) || tt();
      }
      function tt() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function rt(n, t) {
        if (n) {
          if (typeof n == 'string') return Xe(n, t);
          var r = Object.prototype.toString.call(n).slice(8, -1);
          if (
            (r === 'Object' && n.constructor && (r = n.constructor.name),
            r === 'Map' || r === 'Set')
          )
            return Array.from(n);
          if (r === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r))
            return Xe(n, t);
        }
      }
      function Xe(n, t) {
        (t == null || t > n.length) && (t = n.length);
        for (var r = 0, ve = new Array(t); r < t; r++) ve[r] = n[r];
        return ve;
      }
      function nt(n, t) {
        var r =
          n == null
            ? null
            : (typeof Symbol != 'undefined' && n[Symbol.iterator]) || n['@@iterator'];
        if (r != null) {
          var ve = [],
            Se = !0,
            Ee = !1,
            Be,
            Re;
          try {
            for (
              r = r.call(n);
              !(Se = (Be = r.next()).done) && (ve.push(Be.value), !(t && ve.length === t));
              Se = !0
            );
          } catch (ze) {
            (Ee = !0), (Re = ze);
          } finally {
            try {
              !Se && r.return != null && r.return();
            } finally {
              if (Ee) throw Re;
            }
          }
          return ve;
        }
      }
      function at(n) {
        if (Array.isArray(n)) return n;
      }
      var ot = function () {
          var t = (0, K.eL)(),
            r = t.frontmatter,
            ve = (0, e.useState)(!1),
            Se = et(ve, 2),
            Ee = Se[0],
            Be = Se[1],
            Re = (0, K.WF)(),
            ze = Re.themeConfig,
            lt = (0, e.useMemo)(
              function () {
                return ze.socialLinks
                  ? Object.keys(ze.socialLinks)
                      .slice(0, 5)
                      .map(function (Te) {
                        return { icon: Te, link: ze.socialLinks[Te] };
                      })
                  : [];
              },
              [ze.socialLinks],
            );
          return e.createElement(
            'div',
            {
              className: 'dumi-default-header',
              'data-static': Boolean(r.hero) || void 0,
              'data-mobile-active': Ee || void 0,
              onClick: function () {
                return Be(!1);
              },
            },
            e.createElement(
              'div',
              { className: 'dumi-default-header-content' },
              e.createElement(
                'section',
                { className: 'dumi-default-header-left' },
                e.createElement(L.Z, null),
              ),
              e.createElement(
                'section',
                { className: 'dumi-default-header-right' },
                e.createElement(P.Z, null),
                e.createElement(
                  'div',
                  { className: 'dumi-default-header-right-aside' },
                  e.createElement(G.ZP, null),
                  e.createElement(p.Z, null),
                  e.createElement(F.Z, null),
                  ze.prefersColor.switch && e.createElement(d.Z, null),
                  lt.map(function (Te) {
                    return e.createElement(qe, { icon: Te.icon, link: Te.link, key: Te.link });
                  }),
                  e.createElement(g, null),
                ),
              ),
              e.createElement(
                'button',
                {
                  type: 'button',
                  className: 'dumi-default-header-menu-btn',
                  onClick: function (it) {
                    it.stopPropagation(),
                      Be(function (ct) {
                        return !ct;
                      });
                  },
                },
                Ee ? e.createElement(C, null) : e.createElement(U, null),
              ),
            ),
          );
        },
        ut = ot;
    },
    38333: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return A;
        },
      });
      var e = a(3989),
        f = a(70374),
        g = a(50959),
        j = function () {
          var S,
            b = (0, e.eL)(),
            O = b.frontmatter;
          return 'hero' in O
            ? g.createElement(
                'div',
                { className: 'dumi-default-hero' },
                O.hero.title && g.createElement(f.Z, null, O.hero.title),
                O.hero.description &&
                  g.createElement('p', { dangerouslySetInnerHTML: { __html: O.hero.description } }),
                Boolean((S = O.hero.actions) === null || S === void 0 ? void 0 : S.length) &&
                  g.createElement(
                    'div',
                    { className: 'dumi-default-hero-actions' },
                    O.hero.actions.map(function (C) {
                      var x = C.text,
                        J = C.link;
                      return /^(\w+:)\/\/|^(mailto|tel):/.test(J)
                        ? g.createElement(
                            'a',
                            { href: J, target: '_blank', rel: 'noreferrer', key: x },
                            x,
                          )
                        : g.createElement(e.rU, { key: x, to: J }, x);
                    }),
                  ),
              )
            : null;
        },
        A = j;
    },
    8459: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return g;
        },
      });
      var e = a(50959),
        f = function (A) {
          return e.createElement(
            'h1',
            { className: 'dumi-default-hero-title' },
            e.createElement('span', null, A.children),
          );
        },
        g = f;
    },
    80726: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return K;
        },
      });
      var e = a(50959),
        f = Object.defineProperty,
        g = Object.getOwnPropertySymbols,
        j = Object.prototype.hasOwnProperty,
        A = Object.prototype.propertyIsEnumerable,
        E = (d, p, L) =>
          p in d
            ? f(d, p, { enumerable: !0, configurable: !0, writable: !0, value: L })
            : (d[p] = L),
        S = (d, p) => {
          for (var L in p || (p = {})) j.call(p, L) && E(d, L, p[L]);
          if (g) for (var L of g(p)) A.call(p, L) && E(d, L, p[L]);
          return d;
        };
      const b = (d) =>
        e.createElement(
          'svg',
          S({ viewBox: '64 64 896 896' }, d),
          e.createElement('path', {
            d: 'M884 256h-75c-5.1 0-9.9 2.5-12.9 6.6L512 654.2 227.9 262.6c-3-4.1-7.8-6.6-12.9-6.6h-75c-6.5 0-10.3 7.4-6.5 12.7l352.6 486.1c12.8 17.6 39 17.6 51.7 0l352.6-486.1c3.9-5.3.1-12.7-6.4-12.7z',
          }),
        );
      var O =
          'data:image/svg+xml;base64,',
        C = a(3989);
      function x(d, p) {
        return h(d) || v(d, p) || B(d, p) || J();
      }
      function J() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function B(d, p) {
        if (d) {
          if (typeof d == 'string') return I(d, p);
          var L = Object.prototype.toString.call(d).slice(8, -1);
          if (
            (L === 'Object' && d.constructor && (L = d.constructor.name),
            L === 'Map' || L === 'Set')
          )
            return Array.from(d);
          if (L === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(L))
            return I(d, p);
        }
      }
      function I(d, p) {
        (p == null || p > d.length) && (p = d.length);
        for (var L = 0, P = new Array(p); L < p; L++) P[L] = d[L];
        return P;
      }
      function v(d, p) {
        var L =
          d == null
            ? null
            : (typeof Symbol != 'undefined' && d[Symbol.iterator]) || d['@@iterator'];
        if (L != null) {
          var P = [],
            F = !0,
            G = !1,
            te,
            re;
          try {
            for (
              L = L.call(d);
              !(F = (te = L.next()).done) && (P.push(te.value), !(p && P.length === p));
              F = !0
            );
          } catch (se) {
            (G = !0), (re = se);
          } finally {
            try {
              !F && L.return != null && L.return();
            } finally {
              if (G) throw re;
            }
          }
          return P;
        }
      }
      function h(d) {
        if (Array.isArray(d)) return d;
      }
      function D(d) {
        var p = d.pathname,
          L = d.current,
          P = d.target,
          F =
            'base' in L
              ? p.replace(L.base.replace(/\/$/, ''), '') || '/'
              : p.replace(new RegExp(''.concat(L.suffix, '$')), '');
        return 'base' in P
          ? ''
              .concat(P.base.replace(/\/$/, ''))
              .concat(F)
              .replace(/([^/])\/$/, '$1')
          : ''.concat(F).concat(P.suffix);
      }
      var U = function (p) {
          var L = p.locale,
            P = p.current,
            F = (0, C.TH)(),
            G = F.pathname,
            te = (0, e.useState)(function () {
              return D({ pathname: G, current: P, target: L });
            }),
            re = x(te, 2),
            se = re[0],
            ge = re[1];
          return (
            (0, e.useEffect)(
              function () {
                ge(D({ pathname: G, current: P, target: L }));
              },
              [G, P.id, L.id],
            ),
            e.createElement(C.rU, { className: 'dumi-default-lang-switch', to: se }, L.name)
          );
        },
        H = function () {
          var p = (0, C.WF)(),
            L = p.locales,
            P = (0, C.YB)(),
            F = P.locale,
            G = (0, C.bU)();
          return L.length <= 1
            ? null
            : L.length > 2
            ? e.createElement(
                'div',
                { className: 'dumi-default-lang-select' },
                e.createElement(
                  'select',
                  {
                    defaultValue: F,
                    onChange: function (re) {
                      C.m8.push(
                        D({
                          pathname: C.m8.location.pathname,
                          current: G,
                          target: L.find(function (se) {
                            var ge = se.id;
                            return ge === re.target.value;
                          }),
                        }),
                      );
                    },
                  },
                  L.map(function (te) {
                    return e.createElement('option', { key: te.id, value: te.id }, te.name);
                  }),
                ),
                e.createElement(b, null),
              )
            : e.createElement(U, {
                locale: L.find(function (te) {
                  var re = te.id;
                  return re !== F;
                }),
                current: G,
              });
        },
        K = H;
    },
    24618: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return j;
        },
      });
      var e = a(3989),
        f = a(50959),
        g = function () {
          var E = (0, e.WF)(),
            S = E.themeConfig,
            b = (0, e.bU)();
          return f.createElement(
            e.rU,
            { className: 'dumi-default-logo', to: 'base' in b ? b.base : '/' },
            S.logo !== !1 &&
              f.createElement('img', {
                src:
                  S.logo ||
                  'data:image/png;base64,',
                alt: S.name,
              }),
            S.name,
          );
        },
        j = g;
    },
    71368: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return E;
        },
      });
      var e = a(3989),
        f = a(36486),
        g = a(50959);
      function j() {
        return (
          (j = Object.assign
            ? Object.assign.bind()
            : function (S) {
                for (var b = 1; b < arguments.length; b++) {
                  var O = arguments[b];
                  for (var C in O) Object.prototype.hasOwnProperty.call(O, C) && (S[C] = O[C]);
                }
                return S;
              }),
          j.apply(this, arguments)
        );
      }
      var A = function () {
          var b = (0, e.OK)(),
            O = (0, e.TH)(),
            C = O.pathname;
          return g.createElement(
            'ul',
            { className: 'dumi-default-navbar' },
            b.map(function (x) {
              return g.createElement(
                'li',
                { key: x.link },
                /^(\w+:)\/\/|^(mailto|tel):/.test(x.link)
                  ? g.createElement(
                      'a',
                      { href: x.link, target: '_blank', rel: 'noreferrer' },
                      x.title,
                    )
                  : g.createElement(
                      e.rU,
                      j(
                        { to: x.link },
                        C.startsWith(x.activePath || x.link) ? { className: 'active' } : {},
                      ),
                      x.title,
                    ),
              );
            }),
            g.createElement(f.Z, null),
          );
        },
        E = A;
    },
    75022: function (M, N, a) {
      'use strict';
      var e = a(50959),
        f = function () {
          return e.createElement(e.Fragment, null);
        };
      N.Z = f;
    },
    1534: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return B;
        },
      });
      var e = a(3989),
        f = a(50959);
      function g(I, v) {
        return b(I) || S(I, v) || A(I, v) || j();
      }
      function j() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function A(I, v) {
        if (I) {
          if (typeof I == 'string') return E(I, v);
          var h = Object.prototype.toString.call(I).slice(8, -1);
          if (
            (h === 'Object' && I.constructor && (h = I.constructor.name),
            h === 'Map' || h === 'Set')
          )
            return Array.from(I);
          if (h === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(h))
            return E(I, v);
        }
      }
      function E(I, v) {
        (v == null || v > I.length) && (v = I.length);
        for (var h = 0, D = new Array(v); h < v; h++) D[h] = I[h];
        return D;
      }
      function S(I, v) {
        var h =
          I == null
            ? null
            : (typeof Symbol != 'undefined' && I[Symbol.iterator]) || I['@@iterator'];
        if (h != null) {
          var D = [],
            U = !0,
            H = !1,
            K,
            d;
          try {
            for (
              h = h.call(I);
              !(U = (K = h.next()).done) && (D.push(K.value), !(v && D.length === v));
              U = !0
            );
          } catch (p) {
            (H = !0), (d = p);
          } finally {
            try {
              !U && h.return != null && h.return();
            } finally {
              if (H) throw d;
            }
          }
          return D;
        }
      }
      function b(I) {
        if (Array.isArray(I)) return I;
      }
      var O = function () {
          return f.createElement(
            'svg',
            { viewBox: '0 0 14 16' },
            f.createElement('path', {
              d: 'M5.003 6.39v3.594c0 .4.275.674.674.674.4 0 .674-.274.674-.674V1.323h1.997v8.661c0 .4.274.674.674.674s.674-.274.674-.674V1.323h3.295c.399 0 .674-.275.674-.674 0-.4-.275-.649-.674-.649H4.928C3.131 0 1.733 1.398 1.733 3.195S3.206 6.39 5.003 6.39Zm0-5.067v3.72c-1.073 0-1.922-.8-1.922-1.873s.799-1.847 1.922-1.847Zm7.988 11.332H2.73l.8-.674c.274-.2.324-.674.124-.923-.2-.275-.674-.325-.923-.125L.735 12.53c-.275.275-.4.525-.4.874 0 .325.125.674.4.874l1.997 1.597a.829.829 0 0 0 .4.125c.199 0 .398-.075.523-.275.2-.274.2-.723-.125-.923l-.998-.799h10.459c.399 0 .674-.274.674-.674 0-.424-.275-.674-.674-.674Z',
            }),
          );
        },
        C = function () {
          return f.createElement(
            'svg',
            { viewBox: '0 0 14 16' },
            f.createElement('path', {
              d: 'M5.003 6.39v3.594c0 .4.275.674.674.674.4 0 .674-.274.674-.674V1.323h1.997v8.661c0 .4.274.674.674.674s.674-.274.674-.674V1.323h3.295c.399 0 .674-.275.674-.674 0-.4-.275-.649-.674-.649H4.928C3.131 0 1.733 1.398 1.733 3.195S3.206 6.39 5.003 6.39Zm0-5.067v3.72c-1.073 0-1.922-.8-1.922-1.873s.799-1.847 1.922-1.847ZM1.01 12.655h10.26l-.8-.674c-.274-.2-.324-.674-.124-.923.2-.275.674-.325.923-.125l1.997 1.597c.275.275.4.525.4.874 0 .325-.125.674-.4.874l-1.997 1.597a.829.829 0 0 1-.399.125.59.59 0 0 1-.524-.275c-.2-.274-.2-.723.125-.923l.998-.799H1.009c-.399 0-.674-.274-.674-.674 0-.424.275-.674.674-.674Z',
            }),
          );
        },
        x = 'dumi:rtl',
        J = function () {
          var v = (0, f.useState)(!1),
            h = g(v, 2),
            D = h[0],
            U = h[1],
            H = (0, e.WF)(),
            K = H.themeConfig;
          return (
            (0, f.useEffect)(function () {
              localStorage.getItem(x) &&
                (U(!0), document.documentElement.setAttribute('data-direction', 'rtl'));
            }, []),
            K.rtl
              ? f.createElement(
                  'button',
                  {
                    type: 'button',
                    className: 'dumi-default-rtl-switch',
                    onClick: function () {
                      D
                        ? (document.documentElement.removeAttribute('data-direction'),
                          localStorage.removeItem(x))
                        : (document.documentElement.setAttribute('data-direction', 'rtl'),
                          localStorage.setItem(x, '1')),
                        U(!D);
                    },
                    'data-dumi-tooltip': D ? 'RTL' : 'LTR',
                    'data-dumi-tooltip-bottom': !0,
                  },
                  D ? f.createElement(O, null) : f.createElement(C, null),
                )
              : null
          );
        },
        B = J;
    },
    473: function (M, N, a) {
      'use strict';
      a.d(N, {
        ZP: function () {
          return ke;
        },
      });
      var e = a(50959),
        f = Object.defineProperty,
        g = Object.getOwnPropertySymbols,
        j = Object.prototype.hasOwnProperty,
        A = Object.prototype.propertyIsEnumerable,
        E = (o, u, i) =>
          u in o
            ? f(o, u, { enumerable: !0, configurable: !0, writable: !0, value: i })
            : (o[u] = i),
        S = (o, u) => {
          for (var i in u || (u = {})) j.call(u, i) && E(o, i, u[i]);
          if (g) for (var i of g(u)) A.call(u, i) && E(o, i, u[i]);
          return o;
        };
      const b = (o) =>
        e.createElement(
          'svg',
          S({ viewBox: '64 64 896 896' }, o),
          e.createElement('path', {
            d: 'M862 465.3h-81c-4.6 0-9 2-12.1 5.5L550 723.1V160c0-4.4-3.6-8-8-8h-60c-4.4 0-8 3.6-8 8v563.1L255.1 470.8c-3-3.5-7.4-5.5-12.1-5.5h-81c-6.8 0-10.5 8.1-6 13.2L487.9 861a31.96 31.96 0 0 0 48.3 0L868 478.5c4.5-5.2.8-13.2-6-13.2z',
          }),
        );
      var O =
          'data:image/svg+xml;base64,',
        C = Object.defineProperty,
        x = Object.getOwnPropertySymbols,
        J = Object.prototype.hasOwnProperty,
        B = Object.prototype.propertyIsEnumerable,
        I = (o, u, i) =>
          u in o
            ? C(o, u, { enumerable: !0, configurable: !0, writable: !0, value: i })
            : (o[u] = i),
        v = (o, u) => {
          for (var i in u || (u = {})) J.call(u, i) && I(o, i, u[i]);
          if (x) for (var i of x(u)) B.call(u, i) && I(o, i, u[i]);
          return o;
        };
      const h = (o) =>
        e.createElement(
          'svg',
          v({ viewBox: '64 64 896 896' }, o),
          e.createElement('path', {
            d: 'M868 545.5 536.1 163a31.96 31.96 0 0 0-48.3 0L156 545.5a7.97 7.97 0 0 0 6 13.2h81c4.6 0 9-2 12.1-5.5L474 300.9V864c0 4.4 3.6 8 8 8h60c4.4 0 8-3.6 8-8V300.9l218.9 252.3c3 3.5 7.4 5.5 12.1 5.5h81c6.8 0 10.5-8 6-13.2z',
          }),
        );
      var D =
          'data:image/svg+xml;base64,',
        U = Object.defineProperty,
        H = Object.getOwnPropertySymbols,
        K = Object.prototype.hasOwnProperty,
        d = Object.prototype.propertyIsEnumerable,
        p = (o, u, i) =>
          u in o
            ? U(o, u, { enumerable: !0, configurable: !0, writable: !0, value: i })
            : (o[u] = i),
        L = (o, u) => {
          for (var i in u || (u = {})) K.call(u, i) && p(o, i, u[i]);
          if (H) for (var i of H(u)) d.call(u, i) && p(o, i, u[i]);
          return o;
        };
      const P = (o) =>
        e.createElement(
          'svg',
          L({ viewBox: '64 64 896 896' }, o),
          e.createElement('path', {
            d: 'M909.6 854.5 649.9 594.8C690.2 542.7 712 479 712 412c0-80.2-31.3-155.4-87.9-212.1-56.6-56.7-132-87.9-212.1-87.9s-155.5 31.3-212.1 87.9C143.2 256.5 112 331.8 112 412c0 80.1 31.3 155.5 87.9 212.1C256.5 680.8 331.8 712 412 712c67 0 130.6-21.8 182.7-62l259.7 259.6a8.2 8.2 0 0 0 11.6 0l43.6-43.5a8.2 8.2 0 0 0 0-11.6zM570.4 570.4C528 612.7 471.8 636 412 636s-116-23.3-158.4-65.6C211.3 528 188 471.8 188 412s23.3-116.1 65.6-158.4C296 211.3 352.2 188 412 188s116.1 23.2 158.4 65.6S636 352.2 636 412s-23.3 116.1-65.6 158.4z',
          }),
        );
      var F =
          'data:image/svg+xml;base64,',
        G = a(3989),
        te = Object.defineProperty,
        re = Object.getOwnPropertySymbols,
        se = Object.prototype.hasOwnProperty,
        ge = Object.prototype.propertyIsEnumerable,
        Le = (o, u, i) =>
          u in o
            ? te(o, u, { enumerable: !0, configurable: !0, writable: !0, value: i })
            : (o[u] = i),
        De = (o, u) => {
          for (var i in u || (u = {})) se.call(u, i) && Le(o, i, u[i]);
          if (re) for (var i of re(u)) ge.call(u, i) && Le(o, i, u[i]);
          return o;
        };
      const Oe = (o) =>
        e.createElement(
          'svg',
          De({ viewBox: '0 0 1024 1024' }, o),
          e.createElement('path', {
            d: 'm885.2 446.3-.2-.8-112.2-285.1c-5-16.1-19.9-27.2-36.8-27.2H281.2c-17 0-32.1 11.3-36.9 27.6L139.4 443l-.3.7-.2.8c-1.3 4.9-1.7 9.9-1 14.8-.1 1.6-.2 3.2-.2 4.8V830a60.9 60.9 0 0 0 60.8 60.8h627.2c33.5 0 60.8-27.3 60.9-60.8V464.1c0-1.3 0-2.6-.1-3.7.4-4.9 0-9.6-1.3-14.1zm-295.8-43-.3 15.7c-.8 44.9-31.8 75.1-77.1 75.1-22.1 0-41.1-7.1-54.8-20.6S436 441.2 435.6 419l-.3-15.7H229.5L309 210h399.2l81.7 193.3H589.4zm-375 76.8h157.3c24.3 57.1 76 90.8 140.4 90.8 33.7 0 65-9.4 90.3-27.2 22.2-15.6 39.5-37.4 50.7-63.6h156.5V814H214.4V480.1z',
          }),
        );
      var T =
        'data:image/svg+xml;base64,';
      function s(o, u) {
        return y(o) || k(o, u) || c(o, u) || l();
      }
      function l() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function c(o, u) {
        if (o) {
          if (typeof o == 'string') return m(o, u);
          var i = Object.prototype.toString.call(o).slice(8, -1);
          if (
            (i === 'Object' && o.constructor && (i = o.constructor.name),
            i === 'Map' || i === 'Set')
          )
            return Array.from(o);
          if (i === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(i))
            return m(o, u);
        }
      }
      function m(o, u) {
        (u == null || u > o.length) && (u = o.length);
        for (var i = 0, w = new Array(u); i < u; i++) w[i] = o[i];
        return w;
      }
      function k(o, u) {
        var i =
          o == null
            ? null
            : (typeof Symbol != 'undefined' && o[Symbol.iterator]) || o['@@iterator'];
        if (i != null) {
          var w = [],
            V = !0,
            Z = !1,
            W,
            ie;
          try {
            for (
              i = i.call(o);
              !(V = (W = i.next()).done) && (w.push(W.value), !(u && w.length === u));
              V = !0
            );
          } catch (ne) {
            (Z = !0), (ie = ne);
          } finally {
            try {
              !V && i.return != null && i.return();
            } finally {
              if (Z) throw ie;
            }
          }
          return w;
        }
      }
      function y(o) {
        if (Array.isArray(o)) return o;
      }
      var Q = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M5.333 10.667h21.334c.889 0 1.333.444 1.333 1.333s-.444 1.333-1.333 1.333H5.333C4.444 13.333 4 12.89 4 12s.444-1.333 1.333-1.333Z',
            }),
            e.createElement('path', {
              d: 'M13.207 2.667h.126a1.206 1.206 0 0 1 1.2 1.326l-2.413 24.14a1.333 1.333 0 0 1-1.327 1.2h-.126a1.206 1.206 0 0 1-1.2-1.326l2.413-24.14c.068-.682.642-1.2 1.327-1.2Zm8 0h.126a1.206 1.206 0 0 1 1.2 1.326l-2.413 24.14a1.333 1.333 0 0 1-1.327 1.2h-.126a1.206 1.206 0 0 1-1.2-1.326l2.413-24.14c.068-.682.642-1.2 1.327-1.2Z',
            }),
            e.createElement('path', {
              d: 'M5.333 18.667h21.334c.889 0 1.333.444 1.333 1.333s-.444 1.333-1.333 1.333H5.333C4.444 21.333 4 20.89 4 20s.444-1.333 1.333-1.333Z',
            }),
          );
        },
        X = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M9.402 0h14.78L30 6.16V24.5c0 1.933-1.71 3.5-3.589 3.5H9.401C7.524 28 6 26.433 6 24.5v-21C6 1.567 7.523 0 9.402 0ZM23 2v4.183c0 .451.366.817.817.817H28l-5-5Zm3.333 24c.92 0 1.667-.768 1.667-1.714V8.857h-5c-.92 0-1.667-.767-1.667-1.714V2H9.667C8.747 2 8 2.768 8 3.714v20.572C8 25.232 8.746 26 9.667 26h16.666Z',
            }),
          );
        },
        ae = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M6.12 14.589h6.628l1.52 4.004h2.485l-5.938-15.19H8.053L2.115 18.732H4.6l1.52-4.143ZM8.88 6.855c.139-.414.277-.828.415-1.38h.138c0 .138.138.414.414 1.104 0 .138.138.276.138.276 0 .138.829 2.072 2.21 5.938H6.672c1.519-3.866 2.21-5.8 2.21-5.938Zm8.148 2.348h12.705v1.933H17.029V9.203ZM2.115 20.665h27.619v1.933H2.114v-1.933Zm14.914-5.662h12.705v1.933H17.029v-1.933ZM2.115 26.327h27.619v1.933H2.114v-1.933ZM17.029 3.54h12.705v1.934H17.029V3.54Z',
            }),
          );
        },
        oe = function () {
          return e.createElement(
            'svg',
            { viewBox: '0 0 32 32', xmlns: 'http://www.w3.org/2000/svg' },
            e.createElement('path', {
              d: 'M28 6h-5a5 5 0 0 0-10 0H8a2 2 0 0 0-2 2v5a5 5 0 0 0 0 10v5a2 2 0 0 0 2 2h7v-2a3 3 0 0 1 6 0v2h7a2 2 0 0 0 2-2v-7h-2a3 3 0 0 1 0-6h2V8a2 2 0 0 0-2-2Zm-5 12a5 5 0 0 0 5 5v5h-5a5 5 0 0 0-10 0H8v-7H6a3 3 0 0 1 0-6h2V8h7V6a3 3 0 0 1 6 0v2h7v5a5 5 0 0 0-5 5Z',
            }),
          );
        },
        z = { title: Q, page: X, content: ae, demo: oe },
        $ = function (u) {
          return e.createElement(
            e.Fragment,
            null,
            u.texts.map(function (i, w) {
              return e.createElement(
                e.Fragment,
                { key: w },
                i.highlighted ? e.createElement('mark', null, i.text) : i.text,
              );
            }),
          );
        },
        ue = function (u) {
          var i = (0, e.useCallback)(
              function () {
                var ie = 0,
                  ne = [];
                return (
                  u.forEach(function (de) {
                    de.title && ne.push({ type: 'title', value: { title: de.title } }),
                      de.hints.forEach(function (ee) {
                        ne.push({ type: 'hint', activeIndex: ie++, value: ee });
                      });
                  }),
                  [ne, ie]
                );
              },
              [u],
            ),
            w = (0, e.useState)(i),
            V = s(w, 2),
            Z = V[0],
            W = V[1];
          return (
            (0, e.useEffect)(
              function () {
                W(i);
              },
              [u],
            ),
            Z
          );
        },
        Y = function (u) {
          var i = ue(u.data),
            w = s(i, 2),
            V = w[0],
            Z = w[1],
            W = (0, e.useState)(-1),
            ie = s(W, 2),
            ne = ie[0],
            de = ie[1];
          return (
            (0, e.useEffect)(function () {
              var ee = function (je) {
                if (je.key === 'ArrowDown') de((ne + 1) % Z);
                else if (je.key === 'ArrowUp') de((ne + Z - 1) % Z);
                else if (je.key === 'Enter' && ne >= 0) {
                  var ce,
                    pe = V.find(function (he) {
                      return he.type === 'hint' && he.activeIndex === ne;
                    }).value;
                  G.m8.push(pe.link),
                    (ce = u.onItemSelect) === null || ce === void 0 || ce.call(u, pe),
                    document.activeElement.blur();
                }
                ['Escape', 'Enter'].includes(je.key) && de(-1);
              };
              return (
                document.addEventListener('keydown', ee),
                function () {
                  return document.removeEventListener('keydown', ee);
                }
              );
            }),
            e.createElement(
              'div',
              {
                className: 'dumi-default-search-result',
                onMouseEnter: function () {
                  return de(-1);
                },
                onMouseDownCapture: function (Ae) {
                  return Ae.preventDefault();
                },
                onMouseUpCapture: function () {
                  document.activeElement.blur();
                },
              },
              Boolean(u.data.length || u.loading)
                ? e.createElement(
                    'dl',
                    null,
                    V.map(function (ee, Ae) {
                      return ee.type === 'title'
                        ? e.createElement('dt', { key: String(Ae) }, ee.value.title)
                        : e.createElement(
                            'dd',
                            { key: String(Ae) },
                            e.createElement(
                              G.rU,
                              {
                                to: ee.value.link,
                                'data-active': ne === ee.activeIndex || void 0,
                                onClick: function () {
                                  var ce;
                                  return (ce = u.onItemSelect) === null || ce === void 0
                                    ? void 0
                                    : ce.call(u, ee.value);
                                },
                              },
                              e.createElement(z[ee.value.type]),
                              e.createElement(
                                'h4',
                                null,
                                e.createElement($, { texts: ee.value.highlightTitleTexts }),
                              ),
                              e.createElement(
                                'p',
                                null,
                                e.createElement($, { texts: ee.value.highlightTexts }),
                              ),
                            ),
                          );
                    }),
                  )
                : e.createElement(
                    'div',
                    { className: 'dumi-default-search-empty' },
                    e.createElement(Oe, null),
                    e.createElement(G._H, { id: 'search.not.found' }),
                  ),
            )
          );
        },
        R = Y,
        _ = (0, e.forwardRef)(function (o, u) {
          var i = (0, G.YB)(),
            w = (0, e.useRef)(!1),
            V = (0, e.useRef)(null);
          return (
            (0, e.useImperativeHandle)(u, function () {
              return V.current;
            }),
            e.createElement('input', {
              className: 'dumi-default-search-bar-input',
              onCompositionStart: function () {
                return (w.current = !0);
              },
              onCompositionEnd: function (W) {
                (w.current = !1), o.onChange(W.currentTarget.value);
              },
              onFocus: o.onFocus,
              onBlur: o.onBlur,
              onKeyDown: function (W) {
                ['ArrowDown', 'ArrowUp'].includes(W.key) && W.preventDefault(),
                  W.key === 'Escape' && !w.current && W.currentTarget.blur();
              },
              onChange: function (W) {
                setTimeout(function () {
                  w.current || o.onChange(W.target.value);
                }, 1);
              },
              placeholder: i.formatMessage({ id: 'header.search.placeholder' }),
              ref: V,
            })
          );
        }),
        q = function (u) {
          return (
            (0, e.useEffect)(
              function () {
                if (u.visible) document.body.style.overflow = 'hidden';
                else {
                  var i;
                  (document.body.style.overflow = ''),
                    (i = u.onClose) === null || i === void 0 || i.call(u);
                }
              },
              [u.visible],
            ),
            u.visible
              ? e.createElement(
                  'div',
                  { className: 'dumi-default-search-modal' },
                  e.createElement('div', {
                    className: 'dumi-default-search-modal-mask',
                    onClick: u.onMaskClick,
                  }),
                  e.createElement(
                    'div',
                    { className: 'dumi-default-search-modal-content' },
                    u.children,
                  ),
                )
              : null
          );
        },
        le;
      function me(o, u) {
        return Qe(o) || Ie(o, u) || ye(o, u) || Me();
      }
      function Me() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function ye(o, u) {
        if (o) {
          if (typeof o == 'string') return Ce(o, u);
          var i = Object.prototype.toString.call(o).slice(8, -1);
          if (
            (i === 'Object' && o.constructor && (i = o.constructor.name),
            i === 'Map' || i === 'Set')
          )
            return Array.from(o);
          if (i === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(i))
            return Ce(o, u);
        }
      }
      function Ce(o, u) {
        (u == null || u > o.length) && (u = o.length);
        for (var i = 0, w = new Array(u); i < u; i++) w[i] = o[i];
        return w;
      }
      function Ie(o, u) {
        var i =
          o == null
            ? null
            : (typeof Symbol != 'undefined' && o[Symbol.iterator]) || o['@@iterator'];
        if (i != null) {
          var w = [],
            V = !0,
            Z = !1,
            W,
            ie;
          try {
            for (
              i = i.call(o);
              !(V = (W = i.next()).done) && (w.push(W.value), !(u && w.length === u));
              V = !0
            );
          } catch (ne) {
            (Z = !0), (ie = ne);
          } finally {
            try {
              !V && i.return != null && i.return();
            } finally {
              if (Z) throw ie;
            }
          }
          return w;
        }
      }
      function Qe(o) {
        if (Array.isArray(o)) return o;
      }
      var be = /(mac|iphone|ipod|ipad)/i.test(
          typeof navigator != 'undefined'
            ? (le = navigator) === null || le === void 0
              ? void 0
              : le.platform
            : '',
        ),
        xe = function (u) {
          return ['TEXTAREA', 'INPUT'].includes(u.tagName) || u.contentEditable === 'true';
        },
        Ye = function () {
          var u = (0, e.useState)(!1),
            i = me(u, 2),
            w = i[0],
            V = i[1],
            Z = (0, e.useRef)(null),
            W = (0, e.useRef)(null),
            ie = (0, e.useState)('\u2318'),
            ne = me(ie, 2),
            de = ne[0],
            ee = ne[1],
            Ae = (0, G.OO)(),
            je = Ae.keywords,
            ce = Ae.setKeywords,
            pe = Ae.result,
            he = Ae.loading,
            Ve = (0, e.useState)(!1),
            Ze = me(Ve, 2),
            We = Ze[0],
            Ue = Ze[1];
          return (
            (0, e.useEffect)(function () {
              be || ee('Ctrl');
              var fe = function (Ne) {
                if (
                  (((be ? Ne.metaKey : Ne.ctrlKey) && Ne.key === 'k') ||
                    (Ne.key === '/' && !xe(Ne.target))) &&
                  (Ne.preventDefault(), Z.current)
                ) {
                  var Pe = Z.current.getBoundingClientRect(),
                    Fe = Pe.top,
                    _e = Pe.bottom,
                    Ge = Pe.left,
                    Ke = Pe.right,
                    Je = Fe >= 0 && Ge >= 0 && _e <= window.innerHeight && Ke <= window.innerWidth;
                  Je
                    ? Z.current.focus()
                    : (ce(''),
                      Ue(!0),
                      setTimeout(function () {
                        var He;
                        (He = W.current) === null || He === void 0 || He.focus();
                      }));
                }
                Ne.key === 'Escape' && (Ne.preventDefault(), Ue(!1));
              };
              return (
                document.addEventListener('keydown', fe),
                function () {
                  return document.removeEventListener('keydown', fe);
                }
              );
            }, []),
            e.createElement(
              'div',
              { className: 'dumi-default-search-bar' },
              e.createElement(P, { className: 'dumi-default-search-bar-svg' }),
              e.createElement(_, {
                onFocus: function () {
                  return V(!0);
                },
                onBlur: function () {
                  setTimeout(function () {
                    V(!1);
                  }, 1);
                },
                onChange: function (we) {
                  return ce(we);
                },
                ref: Z,
              }),
              e.createElement('span', { className: 'dumi-default-search-shortcut' }, de, ' K'),
              je.trim() &&
                w &&
                (pe.length || !he) &&
                !We &&
                e.createElement(
                  'div',
                  { className: 'dumi-default-search-popover' },
                  e.createElement('section', null, e.createElement(R, { data: pe, loading: he })),
                ),
              e.createElement(
                q,
                {
                  visible: We,
                  onMaskClick: function () {
                    Ue(!1);
                  },
                  onClose: function () {
                    return ce('');
                  },
                },
                e.createElement(
                  'div',
                  { style: { position: 'relative' } },
                  e.createElement(P, { className: 'dumi-default-search-bar-svg' }),
                  e.createElement(_, {
                    onFocus: function () {
                      return V(!0);
                    },
                    onBlur: function () {
                      setTimeout(function () {
                        V(!1);
                      }, 1);
                    },
                    onChange: function (we) {
                      return ce(we);
                    },
                    ref: W,
                  }),
                ),
                e.createElement(R, {
                  data: pe,
                  loading: he,
                  onItemSelect: function () {
                    Ue(!1);
                  },
                }),
                e.createElement(
                  'footer',
                  null,
                  e.createElement(
                    'ul',
                    { className: 'dumi-default-search-modal-commands' },
                    e.createElement(
                      'li',
                      { className: 'dumi-default-search-modal-commands-arrow' },
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-shortcut' },
                        e.createElement(h, {
                          width: '10px',
                          height: '10px',
                          fill: 'rgba(0, 0, 0, 0.45)',
                        }),
                      ),
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-shortcut' },
                        e.createElement(b, {
                          width: '10px',
                          height: '10px',
                          fill: 'rgba(0, 0, 0, 0.45)',
                        }),
                      ),
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-commands-text' },
                        'to navigate',
                      ),
                    ),
                    e.createElement(
                      'li',
                      null,
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-shortcut' },
                        'esc',
                      ),
                      e.createElement(
                        'span',
                        { className: 'dumi-default-search-modal-commands-text' },
                        'to close',
                      ),
                    ),
                  ),
                ),
              ),
            )
          );
        },
        ke = Ye;
    },
    47118: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return A;
        },
      });
      var e = a(3989),
        f = a(68282),
        g = a(50959),
        j = function () {
          var S = (0, e.TH)(),
            b = S.pathname,
            O = (0, e.eL)(),
            C = (0, e.tx)();
          return C
            ? g.createElement(
                'div',
                { className: 'dumi-default-sidebar' },
                C.map(function (x, J) {
                  return g.createElement(
                    'dl',
                    { className: 'dumi-default-sidebar-group', key: String(J) },
                    x.title && g.createElement('dt', null, x.title),
                    x.children.map(function (B) {
                      return g.createElement(
                        'dd',
                        { key: B.link },
                        g.createElement(e.OL, { to: B.link, title: B.title, end: !0 }, B.title),
                        B.link === b && O.frontmatter.toc === 'menu' && g.createElement(f.Z, null),
                      );
                    }),
                  );
                }),
              )
            : null;
        },
        A = j;
    },
    61313: function (M, N, a) {
      'use strict';
      a.d(N, {
        Z: function () {
          return Oe;
        },
      });
      var e = a(90967),
        f = a.n(e),
        g = a(30577),
        j = a.n(g),
        A = a(50959),
        E = a(14557),
        S = a.n(E),
        b = a(39712),
        O = a.n(b),
        C = a(89617),
        x = a.n(C),
        J = a(64698),
        B = a.n(J),
        I = a(62898),
        v = a.n(I),
        h = a(24047),
        D = a.n(h),
        U = O()(function T() {
          S()(this, T);
        }),
        H = (function (T) {
          B()(l, T);
          var s = v()(l);
          function l(c) {
            var m;
            return S()(this, l), (m = s.call(this)), D()(x()(m), 'el', void 0), (m.el = c), m;
          }
          return (
            O()(
              l,
              [
                {
                  key: 'top',
                  get: function () {
                    return this.el.getBoundingClientRect().top;
                  },
                },
                {
                  key: 'outerHeight',
                  get: function () {
                    return this.el.getBoundingClientRect().height;
                  },
                },
                {
                  key: 'scrollTop',
                  get: function () {
                    return this.el.scrollTop;
                  },
                },
                {
                  key: 'scrollHeight',
                  get: function () {
                    return this.el.scrollHeight;
                  },
                },
                {
                  key: 'isScrolledToBottom',
                  value: function () {
                    return this.scrollTop + this.outerHeight >= this.scrollHeight;
                  },
                },
                {
                  key: 'registerScrollEvent',
                  value: function (m) {
                    this.el.addEventListener('scroll', m);
                  },
                },
                {
                  key: 'unregisterScrollEvent',
                  value: function (m) {
                    this.el.removeEventListener('scroll', m);
                  },
                },
              ],
              [
                {
                  key: 'create',
                  value: function (m) {
                    var k = document.querySelector(m);
                    if (!k) throw new Error('element is not found.');
                    return new l(k);
                  },
                },
              ],
            ),
            l
          );
        })(U),
        K = (function (T) {
          B()(l, T);
          var s = v()(l);
          function l() {
            return S()(this, l), s.apply(this, arguments);
          }
          return (
            O()(
              l,
              [
                {
                  key: 'outerHeight',
                  get: function () {
                    return window.innerHeight;
                  },
                },
                {
                  key: 'scrollTop',
                  get: function () {
                    return document.documentElement.scrollTop;
                  },
                },
                {
                  key: 'scrollHeight',
                  get: function () {
                    return document.documentElement.scrollHeight;
                  },
                },
                {
                  key: 'isScrolledToBottom',
                  value: function () {
                    return this.scrollTop + this.outerHeight >= this.scrollHeight;
                  },
                },
                {
                  key: 'registerScrollEvent',
                  value: function (m) {
                    document.addEventListener('scroll', m);
                  },
                },
                {
                  key: 'unregisterScrollEvent',
                  value: function (m) {
                    document.removeEventListener('scroll', m);
                  },
                },
              ],
              [
                {
                  key: 'create',
                  value: function () {
                    return new l();
                  },
                },
              ],
            ),
            l
          );
        })(U),
        d = (function () {
          function T() {
            S()(this, T);
          }
          return (
            O()(T, null, [
              {
                key: 'create',
                value: function (l) {
                  return l ? H.create(l) : K.create();
                },
              },
            ]),
            T
          );
        })(),
        p = function (s) {
          var l = s.sectionRefs,
            c = s.rootSelector,
            m = s.offset,
            k = m === void 0 ? 0 : m,
            y = (0, A.useRef)(null);
          (0, A.useEffect)(
            function () {
              y.current = d.create(c);
            },
            [c],
          );
          var Q = (0, A.useCallback)(
              function () {
                return y.current ? y.current.isScrolledToBottom() : !1;
              },
              [y],
            ),
            X = (0, A.useCallback)(
              function (_) {
                if (!y.current) return !1;
                var q = y.current.scrollTop,
                  le = q + y.current.outerHeight,
                  me = _.getBoundingClientRect(),
                  Me = y.current instanceof H ? q + me.top - y.current.top + k : q + me.top + k,
                  ye = Me + me.height;
                return [Me < le, ye > q].every(function (Ce) {
                  return Ce;
                });
              },
              [y, k],
            ),
            ae = (0, A.useCallback)(
              function () {
                return l.map(function (_) {
                  return _.current ? X(_.current) : !1;
                });
              },
              [X, l],
            ),
            oe = (0, A.useState)([]),
            z = j()(oe, 2),
            $ = z[0],
            ue = z[1],
            Y = (0, A.useMemo)(
              function () {
                return $.findIndex(function (_) {
                  return _;
                });
              },
              [$],
            ),
            R = (0, A.useCallback)(
              function () {
                var _ = Q()
                  ? [].concat(
                      f()(
                        new Array(l.length - 1).fill(!1).map(function (q) {
                          return q;
                        }),
                      ),
                      [!0],
                    )
                  : ae();
                ue(_);
              },
              [ae, Q, l],
            );
          return (
            (0, A.useEffect)(
              function () {
                return (
                  R(),
                  y.current && y.current.registerScrollEvent(R),
                  function () {
                    y.current && y.current.unregisterScrollEvent(R);
                  }
                );
              },
              [R],
            ),
            { elementsStatusInViewport: $, currentElementIndexInViewport: Y }
          );
        },
        L = function (s) {
          var l = s.children,
            c = s.sectionRefs,
            m = s.rootSelector,
            k = s.offset,
            y = p({ sectionRefs: c, rootSelector: m, offset: k }),
            Q = y.elementsStatusInViewport,
            X = y.currentElementIndexInViewport;
          return l({ elementsStatusInViewport: Q, currentElementIndexInViewport: X });
        },
        P = a(3989);
      function F() {
        return (
          (F = Object.assign
            ? Object.assign.bind()
            : function (T) {
                for (var s = 1; s < arguments.length; s++) {
                  var l = arguments[s];
                  for (var c in l) Object.prototype.hasOwnProperty.call(l, c) && (T[c] = l[c]);
                }
                return T;
              }),
          F.apply(this, arguments)
        );
      }
      function G(T, s) {
        return Le(T) || ge(T, s) || re(T, s) || te();
      }
      function te() {
        throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      function re(T, s) {
        if (T) {
          if (typeof T == 'string') return se(T, s);
          var l = Object.prototype.toString.call(T).slice(8, -1);
          if (
            (l === 'Object' && T.constructor && (l = T.constructor.name),
            l === 'Map' || l === 'Set')
          )
            return Array.from(T);
          if (l === 'Arguments' || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(l))
            return se(T, s);
        }
      }
      function se(T, s) {
        (s == null || s > T.length) && (s = T.length);
        for (var l = 0, c = new Array(s); l < s; l++) c[l] = T[l];
        return c;
      }
      function ge(T, s) {
        var l =
          T == null
            ? null
            : (typeof Symbol != 'undefined' && T[Symbol.iterator]) || T['@@iterator'];
        if (l != null) {
          var c = [],
            m = !0,
            k = !1,
            y,
            Q;
          try {
            for (
              l = l.call(T);
              !(m = (y = l.next()).done) && (c.push(y.value), !(s && c.length === s));
              m = !0
            );
          } catch (X) {
            (k = !0), (Q = X);
          } finally {
            try {
              !m && l.return != null && l.return();
            } finally {
              if (k) throw Q;
            }
          }
          return c;
        }
      }
      function Le(T) {
        if (Array.isArray(T)) return T;
      }
      var De = function () {
          var s = (0, P.TH)(),
            l = s.pathname,
            c = s.search,
            m = (0, P.eL)(),
            k = (0, P.WF)(),
            y = k.loading,
            Q = (0, A.useRef)(0),
            X = (0, A.useState)([]),
            ae = G(X, 2),
            oe = ae[0],
            z = ae[1],
            $ = m.toc.filter(function (ue) {
              var Y = ue.depth;
              return Y > 1 && Y < 4;
            });
          return (
            (0, A.useEffect)(
              function () {
                if (!y) {
                  var ue = $.map(function (Y) {
                    var R = Y.id;
                    return { current: document.getElementById(R) };
                  });
                  z(ue);
                }
              },
              [l, c, y],
            ),
            oe.length
              ? A.createElement(L, { sectionRefs: oe }, function (ue) {
                  var Y = ue.currentElementIndexInViewport;
                  return (
                    Y > -1 && (Q.current = Y),
                    A.createElement(
                      'ul',
                      { className: 'dumi-default-toc' },
                      $.filter(function (R) {
                        var _ = R.depth;
                        return _ > 1 && _ < 4;
                      }).map(function (R, _) {
                        var q = '#'.concat(encodeURIComponent(R.id)),
                          le = Y > -1 ? Y : Q.current;
                        return A.createElement(
                          'li',
                          { key: R.id, 'data-depth': R.depth },
                          A.createElement(
                            P.rU,
                            F({ to: q, title: R.title }, le === _ ? { className: 'active' } : {}),
                            R.title,
                          ),
                        );
                      }),
                    )
                  );
                })
              : null
          );
        },
        Oe = De;
    },
    80169: function (M, N, a) {
      var e = a(57363);
      function f(g) {
        if (Array.isArray(g)) return e(g);
      }
      (M.exports = f), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    89617: function (M) {
      function N(a) {
        if (a === void 0)
          throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
        return a;
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    14557: function (M) {
      function N(a, e) {
        if (!(a instanceof e)) throw new TypeError('Cannot call a class as a function');
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    39712: function (M, N, a) {
      var e = a(81493);
      function f(j, A) {
        for (var E = 0; E < A.length; E++) {
          var S = A[E];
          (S.enumerable = S.enumerable || !1),
            (S.configurable = !0),
            'value' in S && (S.writable = !0),
            Object.defineProperty(j, e(S.key), S);
        }
      }
      function g(j, A, E) {
        return (
          A && f(j.prototype, A),
          E && f(j, E),
          Object.defineProperty(j, 'prototype', { writable: !1 }),
          j
        );
      }
      (M.exports = g), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    62898: function (M, N, a) {
      var e = a(37970),
        f = a(51621),
        g = a(76973);
      function j(A) {
        var E = f();
        return function () {
          var b = e(A),
            O;
          if (E) {
            var C = e(this).constructor;
            O = Reflect.construct(b, arguments, C);
          } else O = b.apply(this, arguments);
          return g(this, O);
        };
      }
      (M.exports = j), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    37970: function (M) {
      function N(a) {
        return (
          (M.exports = N =
            Object.setPrototypeOf
              ? Object.getPrototypeOf.bind()
              : function (f) {
                  return f.__proto__ || Object.getPrototypeOf(f);
                }),
          (M.exports.__esModule = !0),
          (M.exports.default = M.exports),
          N(a)
        );
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    64698: function (M, N, a) {
      var e = a(47047);
      function f(g, j) {
        if (typeof j != 'function' && j !== null)
          throw new TypeError('Super expression must either be null or a function');
        (g.prototype = Object.create(j && j.prototype, {
          constructor: { value: g, writable: !0, configurable: !0 },
        })),
          Object.defineProperty(g, 'prototype', { writable: !1 }),
          j && e(g, j);
      }
      (M.exports = f), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    51621: function (M) {
      function N() {
        if (typeof Reflect == 'undefined' || !Reflect.construct || Reflect.construct.sham)
          return !1;
        if (typeof Proxy == 'function') return !0;
        try {
          return Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], function () {})), !0;
        } catch (a) {
          return !1;
        }
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    50823: function (M) {
      function N(a) {
        if ((typeof Symbol != 'undefined' && a[Symbol.iterator] != null) || a['@@iterator'] != null)
          return Array.from(a);
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    58313: function (M) {
      function N() {
        throw new TypeError(`Invalid attempt to spread non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    76973: function (M, N, a) {
      var e = a(31077).default,
        f = a(89617);
      function g(j, A) {
        if (A && (e(A) === 'object' || typeof A == 'function')) return A;
        if (A !== void 0)
          throw new TypeError('Derived constructors may only return object or undefined');
        return f(j);
      }
      (M.exports = g), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    47047: function (M) {
      function N(a, e) {
        return (
          (M.exports = N =
            Object.setPrototypeOf
              ? Object.setPrototypeOf.bind()
              : function (g, j) {
                  return (g.__proto__ = j), g;
                }),
          (M.exports.__esModule = !0),
          (M.exports.default = M.exports),
          N(a, e)
        );
      }
      (M.exports = N), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
    90967: function (M, N, a) {
      var e = a(80169),
        f = a(50823),
        g = a(93897),
        j = a(58313);
      function A(E) {
        return e(E) || f(E) || g(E) || j();
      }
      (M.exports = A), (M.exports.__esModule = !0), (M.exports.default = M.exports);
    },
  },
]);
